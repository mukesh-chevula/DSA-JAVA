...
/**
 * A binary max-heap implementation using an explicit binary tree structure.
...
public class BinaryMaxHeap {
    ...
    /**
     * Retrieves, but does not remove, the maximum element of this heap.
    ...
    /**
     * Retrieves and removes the maximum element of this heap.
     * @return The maximum element.
     * @throws NoSuchElementException if the heap is empty.
     */
    public int poll() {
    ...
    private void heapifyDown(Node node) {
        ...
        while (current.left != null) { // While there is at least a left child
            Node largerChild = current.left;
            if (current.right != null && current.right.value > largerChild.value) {
                largerChild = current.right;
            }

            if (current.value > largerChild.value) {
                break; // Heap property is satisfied
            } else {
                swapValues(current, largerChild);
                current = largerChild; // Move down to the child
            }
        }
    }
